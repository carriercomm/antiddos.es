...
<?php $tail = '-enable'; if(substr($row->getName(), (-1) * strlen($tail)) == $tail): ?>
  ...
  <div class="selecdiv s1">
	...
	<div style="position: relative; top: -18px; left: 28px;" onclick="var $cb = $('#cart_item_addons_<?php echo $row->getName() ?>'); $cb.click(); "><b><?php echo $form['addons'][$orig_name]->renderLabel(null, array('style' => 'cursor: pointer;')) ?>:</b></div>
  </div>
...
<?php else: ?>
  ...
  $(function () {
    if(
      $('#cart_item_addons_<?php echo $row->getWidget()->generateId($row->getName()) ?>-enable').is(':checkbox')
      && !$('#cart_item_addons_<?php echo $row->getWidget()->generateId($row->getName()) ?>-enable').is(':checked')
