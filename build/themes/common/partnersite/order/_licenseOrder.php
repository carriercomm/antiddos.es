  ...
  <td width=32><img src="<?php echo solar_image_url('minilicense2.png') ?>" width=32></td>
  <td align=left><b><?php t('Licenses')?></b><br><?php echo link_to(__($order->isItemAssigned() ? $order->item['name'] : 'N/A'), '@order_show?id=' . $order->id, 'class="link"') ?></td>

  <td class="int2" width=120 align=center><?php echo price_ex($order->price) ?></td>
  <td class="int2" width=140 align=center><font class="status <?php echo status($order->status) ?>"><?php t(status($order->status)) ?><font></td>
