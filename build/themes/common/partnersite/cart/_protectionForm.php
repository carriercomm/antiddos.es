...
var max_websites = <?php echo $form->getProduct()->max_websites ?>;
var max_domains = <?php echo $form->getProduct()->max_domains ?>;
...
function addWebsite(num, name)
{
  var r = $.ajax({
    type: 'GET',
    url: '<?php echo url_for('@cart_protection_website_add?product_id=' . $form->getProduct()->id) ?>' + '?num=' + num + '&name=' + name,
    async: false
  }).responseText;
...
function addDomain(websitenum, num, id)
{
  var r = $.ajax({
    type: 'GET',
    url: '<?php echo url_for('@cart_protection_domain_add?product_id=' . $form->getProduct()->id) ?>' + '?website_num=' + websitenum + '&num=' + num +'&id=' + id,
    async: false
  }).responseText;
...
<?php include_partial('cart/addWebsite', array('form' => $form, 'num' => $n)) ?>
<?php endforeach ?>
